...
const DifferenceRow: React.FC<{ label: string; value1: React.ReactNode; value2: React.ReactNode }> = ({ label, value1, value2 }) => (
  <div className="flex justify-between items-center py-2 border-b last:border-b-0">
    ...
    <span className="text-sm font-medium text-gray-900 text-center w-1/3">{value1}</span>
    <span className="text-sm font-medium text-gray-900 text-center w-1/3">{value2}</span>
  </div>
);

const KeyDifferences: React.FC<KeyDifferencesProps> = ({ plan1, plan2 }) => {
  return (
    <div className="bg-white p-4 md:p-6 rounded-lg shadow-md border border-gray-200 mb-6 md:mb-8">
      ...
      <div className="flex justify-between items-center py-2 border-b font-semibold">
        <span className="text-sm text-gray-800 w-1/3">Feature</span>
        <span className="text-sm text-gray-800 text-center w-1/3" title={plan1.planName}>{plan1.planNameShort || 'Plan 1'}</span>
        <span className="text-sm text-gray-800 text-center w-1/3" title={plan2.planName}>{plan2.planNameShort || 'Plan 2'}</span>
      </div>
